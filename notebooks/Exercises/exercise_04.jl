...
begin 
	...
	<p style="padding-bottom:1cm"> </p>
	<p align=center style="font-size:25px; font-family:family:Georgia"> <b> Exercise 04
	</b> <p>
	<p style="padding-bottom:1cm"> </p>
...
# ╔═╡ df0cd794-162c-4c3c-b63b-0b0a48bc890a
...
Markdown.parse("""
- Suppose the yield curve is as shown below. Calculate the full price of a $(T_Oct2021)-year Treasury note. Suppose the Treasury note has a coupon rate of $(c_Oct2021*100)% (paid-semiannually) and face value of \\\$$(F_Oct2021).
...
Markdown.parse("
!!! hint
	- The semi-annual coupon cash flows are ``\$C=\\frac{$(c_Oct2021)}{2}\\times $(F_Oct2021) = $(C_Oct2021[1])\$``.
	
	- The full price of the Treasury note is:
	
	``\$ P = \\frac{$(C_Oct2021[1])}{\\left(1+\\frac{$(r_Oct2021[1])\\%}{2}\\right)^{2\\times 0.5}} + \\frac{$(C_Oct2021[2])}{\\left(1+\\frac{$(r_Oct2021[2])\\%}{2}\\right)^{2\\times 1.0}} + \\frac{$(C_Oct2021[3])}{\\left(1+\\frac{$(r_Oct2021[3])\\%}{2}\\right)^{2\\times 1.5}} + \\frac{$(C_Oct2021[4])}{\\left(1+\\frac{$(r_Oct2021[4])\\%}{2}\\right)^{2\\times 2.0}}  \$``
...
begin
	T_Oct2022 = 2.5
	dt_Oct2022 = collect(0.5:0.5:T_Oct2022)
	F_Oct2022 = 1000
	c_Oct2022 = 0.05
	r_Oct2022 = [0.50, 0.98, 1.10, 1.37, 1.57]
	C_Oct2022 = (c_Oct2022)/2*F_Oct2022 .* ones(length(r_Oct2022))
	C_Oct2022[end] += F_Oct2022 
	DT_Oct2022 = 1 ./ (1 .+ r_Oct2022./200).^(dt_Oct2022)
	P_Oct2022 = sum(C_Oct2022./(1 .+ r_Oct2022./200).^(dt_Oct2022))
	display("")
end

# ╔═╡ cd00e881-9e27-44ac-8334-d64255bbc3be
Markdown.parse("
- Suppose you are given discount factors out to  $(T_Oct2022) years as shown below. Calculate the price of a $(T_Oct2022)-year Treasury note with coupon rate of $(c_Oct2022*100) percent (paid-semiannually). Assume \\\$$(F_Oct2022) par value.

 ``t`` [years] | ``D(t)`` 
 :---|:----------
 0.5	| $(roundmult(DT_Oct2022[1],1e-4))
 1.0	| $(roundmult(DT_Oct2022[2],1e-4))
 1.5	| $(roundmult(DT_Oct2022[3],1e-4))
 2.0	| $(roundmult(DT_Oct2022[4],1e-4))
 2.5	| $(roundmult(DT_Oct2022[5],1e-4))")

# ╔═╡ c428cca3-52c1-4e98-86b0-053518a81c46
Markdown.parse("
!!! hint
	- The price of the Treasury note is:
	
	``\$P = C\\times D(0.5) + C\\times D(1.0) + C\\times D(1.5) + C\\times D(2.0) + (C+F)\\times D(2.5)\$``
...
md"""
- Use Boot-Strapping to calculate the (zero-coupon) yield curve.
"""
...
begin
	...
	r5 = [0.095, 0.213, 0.437, 0.643, 0.806, 0.965, 1.063, 1.155, 1.248, 1.344].*2
	C5 = r5./200 .* F5
	D5 = zeros(length(dt5))
	D5[1] = F5/(F5+C5[1])
...
begin
	...
	r6 = [0.0, 0.025, 0.02125, 0.01125, 0.01875, 0.0177, 0.02375, 0.02625, 0.025, 0.02].*100
	C6 = r6./200 .* F6
	D6 = zeros(length(dt6))
	D6[1] = P6[1]/(F6+C6[1])
...
Bootstrap the zero-coupon yield curve out to five years (in 6-month intervals).
  ...
  T   | Maturity date   | Coupon rate  | Price     |
  :---|:----------------|:-------------|:----------|
  0.5 | 1/30/2020       | $(r6[1])     | $(P6[1])  |
  1   | 6/30/2020       | $(r6[2])     | $(P6[2])  |
  1.5 | 1/31/2021       | $(r6[3])     | $(P6[3])  |
  2   | 6/30/2021       | $(r6[4])     | $(P6[4])  |
  2.5 | 1/31/2022       | $(r6[5])     | $(P6[5])  |
  3   | 6/30/2022       | $(r6[6])     | $(P6[6])  |
  3.5 | 1/31/2023       | $(r6[7])     | $(P6[7])  |
  4   | 6/30/2023       | $(r6[8])     | $(P6[8])  |
  4.5 | 1/31/2024       | $(r6[9])     | $(P6[9])  |
  5   | 6/30/2024       | $(r6[10])    | $(P6[10]) |
""")
...
md"""
- Replicate a 1-year bond, with a face value of $100 and a coupon rate of 10% using the following set of bonds.
...
!!! hint
	...
	:--------------------------|:--------|:-----------|:----------|:-----------
	1-yr bond, 5% coupon rate  | 1       | ?          | 5         | 105
	
	Let's set up a replicating portfolio
	
	Position                   | Units   | t=0        | t=0.5     | t=1
	:--------------------------|:--------|:-----------|:----------|:-----------
	1-yr bond, 10% coupon rate | 1       | $P_1$      | 5.0       | 105.0
	...
	Position                   | Units   | t=0        | t=0.5     | t=1
	:--------------------------|:--------|:-----------|:----------|:-----------
	1-yr bond, 5% coupon rate  | 1       | $P_1$          | 5.0       | 105.0
	...
	- Since, we are not matching the cash flows of 5.0 at t=0.5 and 105.0 at t=1, let's adjust the position in the 1-year 2.5% coupon bond.
	- To match the 105.0, we more than one unit. Specifically, we need $\frac{105.0}{101.25}=1.037037$ units.
	- The coupon cash flow at t=0.5 is then $1.037037 \textrm{ units} \times 1.25=1.29629625$.
	- To match the cash flow of 5.0, we use the 0.5-year zero coupon bond.
